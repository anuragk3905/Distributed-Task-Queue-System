...
A scalable, distributed task queue architecture built with Node.js, Express, BullMQ, and React. It features real-time monitoring via WebSockets, a robust dashboard for queue management, and modular architecture using Turborepo.
...
1. **API Service (`apps/api`)**: The core Express.js backend. Handles user authentication (JWT), REST endpoints for job creation, workflows, and serves the BullBoard admin panel. Uses Mongoose to connect to MongoDB and Upstash Redis for message brokering.
...
- **Databases**: MongoDB (Main application data, users), Upstash Redis (Task queues, caching).
...
You will need to create a `.env` file in the root directory (or inside each specific app) using this template:

**For `apps/api` and `apps/worker` and `apps/socket`:**
```env
PORT=3000
MONGODB_URI=mongodb+srv://<username>:<password>@cluster.mongodb.net/<dbname>
UPSTASH_REDIS_URL=rediss://<username>:<password>@<redis-url>
...
### 4. Running Locally
Because this is a Turborepo workspace, you can easily spin up the entire cluster or run individual services.
...
The Socket server will be running on `http://localhost:4000`.
